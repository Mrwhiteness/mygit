...
import com.android.instructionbook.model.ColumnInfo;
import com.android.instructionbook.util.Utils;
...
public class OperationManualDetail extends Activity implements
	...
	private void setRelatedInfo() {
		this.mRelatedInfoLayout.setVisibility(View.GONE);
		if (this.mPageNameList.size() > 1) {
			getRelatedPages();
			this.mRelatedInfoLayout.setVisibility(View.VISIBLE);
	...
	private void setUpViews() {
		this.mPreviousItem = findViewById(R.id.previousItem);
		this.mContentTitle = findViewById(R.id.content_title);
		this.mMainScrollView = findViewById(R.id.mainScroll);
		this.mNextItem = findViewById(R.id.nextItem);
		this.mWebView = findViewById(R.id.webview);
		this.mRelatedInfoLayout = findViewById(R.id.relatedInfo);
		this.mRelatedGridView = findViewById(R.id.relatedListView);
		initWebView();
	}
	...
	private void updateTitleView() {
		...
		if (this.mPageNameList.size() == 1) {
			this.mPreviousItem.setVisibility(View.INVISIBLE);
			this.mNextItem.setVisibility(View.INVISIBLE);
			return;
		}
		int i = this.mPageNameList.indexOf(this.mCurPageName);
		if (i == 0) {
			this.mPreviousItem.setVisibility(View.INVISIBLE);
			this.mNextItem.setVisibility(View.VISIBLE);
			return;
		}
		if (i == -1 + this.mPageNameList.size()) {
			this.mPreviousItem.setVisibility(View.VISIBLE);
			this.mNextItem.setVisibility(View.INVISIBLE);
			return;
		}
		this.mPreviousItem.setVisibility(View.VISIBLE);
		...
		public View getView(int paramInt, View paramView,
			...
			if (paramView == null) {
				paramView = OperationManualDetail.this.mInflater.inflate(
						R.layout.related_list_item, null);
				localViewHolder = new ViewHolder();
				localViewHolder.mTextTitle = paramView
						.findViewById(R.id.txtview_relaInfo);
				paramView.setTag(localViewHolder);
			}
			while (true) {
				localViewHolder = (ViewHolder) paramView
						.getTag();
